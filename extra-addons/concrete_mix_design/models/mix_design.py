...
class ConcreteDesign(models.Model):
    ...
    # _inherit = "lerm.eln"
    _description = 'Mix Design'
    _rec_name = "name"

    name1 = fields.Char("Name",default="CONCRETE MIX DESIGN")
    name = fields.Char(string="Name", required=True, copy=False, readonly=True, default='New')
    mix_parent_id = fields.Many2one('lerm.mix.design.parent') 
    ...
    def create(self, vals):
        if vals.get("name", "New") == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("mechanical.concrete.design.seq") or "New"
            
        record = super().create(vals)
        if record.mix_parent_id:
            self.env['mix.lines'].sudo().create({
                'parent_id': record.mix_parent_id.id,
                'concrete_mix_id': record.id
            })
        return record
    ...
    def save_mix_design(self):
        ...
        return {
            'view_mode': 'form',
            'res_model': "lerm.mix.design.parent",
            'type': 'ir.actions.act_window',
            'target': 'current',
            'res_id': self.mix_parent_id.id,
